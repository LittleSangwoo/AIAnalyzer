...
namespace AIAnalyzer.Services
{
    ...
    public class AiService : IAiService
    {
        ...
        private async Task<string> GetSberAccessTokenAsync(string authKey, string scope = "GIGACHAT_API_PERS")
        {
            var client = _httpClientFactory.CreateClient();

            // Console.WriteLine($"DEBUG: Authorization header string: Basic {authKey}");
            ...
            string authUrl = _configuration["AiSettings:GigaChat:AuthUrl"] ?? "https://ngw.devices.sberbank.ru:9443/api/v2/oauth";
        ...
        public async Task<string> GenerateRecommendationAsync(List<QuestionStatDto> targetQuestions, string promptType, string modelProvider, string userApiKey)
        {
            ...
            for (int i = 0; i < chunks.Count; i++)
            {
                ...
                string intermediatePrompt = "Ты — аналитик. Твоя задача кратко выписать основные паттерны ошибок и проблемные темы из этой части данных. Не пиши длинных вступлений, только суть. Это пойдет в финальный отчет. НИ В КОЕМ СЛУЧАЕ НЕ УПОМИНАЙ ID ДРУГИХ ВОПРОСОВ В КОРРЕКТИРОВКАХ ДЛЯ КОНКРЕТНОГО ВОПРОСА, упоминай только id разбираемого вопроса";
...
Отвечай структурировано, в Markdown.",
        ...
        private async Task<string> ProcessSingleBatchAsync(IEnumerable<QuestionStatDto> targetQuestions, string promptType, string modelProvider, string userApiKey)
        {
            ...
            string systemPrompt = promptType.ToLower() switch
            {
                ...
                Отвечай в формате четкого списка с заголовками. Твоя цель — сделать тест проверяющим знания, а не умение разгадывать ребусы или угадывать регистр.НИ В КОЕМ СЛУЧАЕ НЕ УПОМИНАЙ ID ДРУГИХ ВОПРОСОВ В КОРРЕКТИРОВКАХ ДЛЯ КОНКРЕТНОГО ВОПРОСА, упоминай только id разбираемого вопроса",
        ...
        {
            string systemPrompt = "Ты — умный помощник преподавателя и аналитик учебных курсов. Отвечай на вопросы пользователя профессионально, четко, по делу и с примерами. Обязательно используй Markdown (заголовки, списки, жирный текст) для удобства чтения. упоминай id разбираемого вопроса";
        ...
        private async Task<string> SendApiRequestAsync(string systemPrompt, string userContent, string modelProvider, string userApiKey)
        {
            ...
            if (File.Exists(providersFilePath))
            {
                ...
                if (provider != null)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(finalApiKey)) finalApiKey = provider.ApiKey;
            ...
            // 2. РЕЗЕРВНЫЙ ПОИСК: Если в JSON не нашли
            ...
            // 3. АВТОРИЗАЦИЯ СБЕРА (Вызывается ровно один раз)
            if (isGigaChat)
            {
                if (string.IsNullOrWhiteSpace(finalApiKey) || finalApiKey == "-")
                {
                    finalApiKey = await GetSberAccessTokenAsync(_configuration["AiSettings:GigaChat:AuthorizationKey"], gigaChatScope);
                }
                else
                {
                    finalApiKey = await GetSberAccessTokenAsync(finalApiKey, gigaChatScope);
                }
            }

            // Если ключ всё ещё пустой
            if (string.IsNullOrWhiteSpace(finalApiKey))
            {
